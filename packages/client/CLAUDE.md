...
- 신규 코드에서 `useCallback`, `useMemo`, `React.memo`를 기본 최적화 수단으로 사용하지 않는다.
- 메모이제이션은 우선 React Compiler에 맡긴다.
- 아래 경우에만 예외적으로 수동 memoization을 허용한다.
  - 측정된 렌더링 병목이 있고 수동 최적화 효과가 확인된 경우
  - React Compiler가 opt-out된 컴포넌트에서, 또는 외부 라이브러리가 콜백의 참조 안정성을 직접 요구하는 경우
  - 기존 코드는 제거를 권장하되, 제거 시 동작 또는 성능 회귀가 우려되는 경우에만 유지한다.
- 예외를 추가할 때는 코드 근처에 이유를 짧게 남긴다.
