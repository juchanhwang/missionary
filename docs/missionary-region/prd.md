...
- **사용자**: 모든 사용자
- **필요 API**: `GET /regions?missionGroupId=&missionaryId=&query=&limit=&offset=` (신규)
...
- **페이지네이션**:
  ...
  - 필터/검색 변경 시 1페이지로 리셋
- **URL 상태**: `?missionGroupId=&missionaryId=&query=&page=` — 필터/검색/페이지 상태를 URL 쿼리 파라미터로 유지하여 새로고침 및 링크 공유 가능.
...
| 필터/검색 상태 | **URL 쿼리 파라미터** (`?missionGroupId=&missionaryId=&query=&page=`) | 새로고침 유지, 링크 공유 가능 |
