...
| 승인 | PO 협의 완료 (2026-03-19) — v1.9 확정 |
| 참조 PRD | prd.md (v0.8) |
...
| v1.8 | 용어 전면 정비 — 필터/폼/모달 내 "선교(Missionary)" → "차수" 반영; 파일 구조 섹션 삭제(fe-plan.md로 이관); 개발 구현 가이드 섹션 삭제(fe-plan.md로 이관); 참조 PRD v0.8로 갱신 |
| v1.9 | **[페이지네이션 추가]** 테이블 하단 Pagination 컴포넌트 설계 확정(20건/페이지, 유저 관리 패턴 일치); URL `page` 파라미터 추가; 확정 사항 #1 업데이트 |
...
**결정**: SearchBox를 필터 행 **맨 왼쪽**에 배치 → cascade 필터(선교 그룹 → 차수)는 오른쪽

| 이전 (v1.4) | 현재 (v1.5) |
|------------|------------|
| `[선교 그룹 ▼] [차수 ▼] [🔍 검색 flex-1]` | `[🔍 검색 flex-1] [선교 그룹 ▼] [차수 ▼]` |
...
### [v1.9 신규] 페이지네이션: 20건/페이지

**결정**: 테이블 하단에 `Pagination` 컴포넌트 배치 — **유저 관리(`/users`) 패턴 동일 적용**

| 항목 | 내용 |
|------|------|
| 페이지 크기 | 20건 (PAGE_SIZE = 20) |
| URL 파라미터 | `?page=` (1-based, 기본 1) |
| 표시 형식 | 좌: `1 - 20 / 전체 85건` · 우: `[◀] [1][2][3][4][5] [▶]` |
| 페이지 그룹 | 5개 단위 (유저 관리 패턴 동일) |
| 필터/검색 변경 | → `page=1` 강제 리셋 (cascade) |
| 단일 페이지 | 전체 건수 ≤ 20이면 Pagination 미표시 |

**근거**: 데이터 증가 시 성능·UX 보장. 유저 관리와 동일 컴포넌트/패턴으로 Admin 내 일관성 확보.
...
**결정**: 모달 (슬라이드 패널 아님)
근거: 총 필드 수 8개(선교 그룹/차수 포함)로 단순, 슬라이드 패널은 필드 10+일 때 유리

---

### [v1.3 유지] 수정 모달의 선교 그룹/차수: 읽기 전용 처리
...
| 차수 Select | "차수" | "전체" | URL: `missionaryId`, 선교 그룹 선택 시 해당 그룹 소속 차수만 표시 |

**필터 연동 규칙**:
- 선교 그룹 변경 시 → 차수 Select 리셋 ("전체"로 복귀)
...
테이블 카드 하단 footer:
┌──────────────────────────────────────────────────────────────────────┐
│  1 - 20 / 전체 85건          [◀] [1] [2] [3] [4] [5] [▶]           │
└──────────────────────────────────────────────────────────────────────┘
```

| 요소 | 설명 |
|------|------|
| 좌측 범위 표시 | `{시작} - {끝} / 전체 {total}건` |
| `Pagination` | DS `Pagination` 컴포넌트 — `currentPage`, `totalPages`, `onPageChange` |
| 이전/다음 버튼 | 첫/마지막 페이지에서 `disabled` (`disabled:opacity-30`) |
| 페이지 번호 | 5개 그룹 단위. 현재 페이지: `bg-gray-900 text-white font-bold` |
| 미표시 조건 | `totalCount ≤ PAGE_SIZE`(20) 이면 Pagination 렌더링 생략 |

---

### 2-4. 등록/수정 모달 [v1.4: 등록 모달 자동 채움 조건 변경]
...
| 필드 | 컴포넌트 | 필수 | 기본값 | 비고 |
|------|---------|:----:|--------|------|
| 선교 그룹 | `Select` | Y | 필터값 자동 채움 (전체면 빈값) | 옵션에 카테고리 라벨, 변경 시 차수 Select 리셋 |
| 차수 | `Select` | Y | 필터값 자동 채움 (전체면 빈값) | 선교 그룹 선택 후 목록 활성화 |
| 이름 | `InputField` | Y | — | |
| 방문목적 | `InputField` | N | — | |
| 목사명 | `InputField` | N | — | |
...
| 차수 | 편집 가능 Select | `disabled` + Lock 아이콘 + tooltip |
...
└── page: number | null            — 현재 페이지 (null = 1, 필터/검색 변경 시 1로 리셋)
...
```
Pagination 버튼 클릭 (페이지 번호 / 이전 / 다음)
  → URL 파라미터 page=N 업데이트
  → GET /regions?missionGroupId=&missionaryId=&query=&page=N&pageSize=20 호출
  → 로딩 중: Skeleton
  → 결과: 테이블 갱신, 범위 표시 갱신 (예: 21 - 40 / 전체 85건)

필터/검색 변경 시
  → page=1 강제 리셋 후 API 호출
```
...
| 등록 모달 "전체" 상태 오픈 | 선교 그룹/차수 빈값, 사용자 직접 선택 필수 |
...
## 7. 확정 사항 (Resolved) [v1.5 최종]

| # | 질문 | 결정 |
|---|------|------|
| 1 | 페이지네이션 필요 여부? | **[v1.9 변경] 필요 — 20건/페이지, 유저 관리 Pagination 컴포넌트 패턴 동일 적용** |
| 2 | 차수 Select "전체" 옵션 필요? | **필요 — 선교 그룹/차수 모두 "전체" 기본값** |
...
| 7 | 수정 모달 선교 그룹/차수 | **disabled + Lock 아이콘 + tooltip** |
